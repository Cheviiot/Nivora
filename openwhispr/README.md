...
## Вставка текста и Wayland

Собственный launcher upstream (`open-whispr`) форсирует
`--ozone-platform=x11` под Wayland-сессией — рендеринг всегда идёт через
X11/XWayland, где вставка распознанного текста работает через встроенный
XTest-биндинг (`libX11`/`libXtst`/`libXi`, уже жёсткие зависимости пакета)
без какой-либо дополнительной настройки.

Если вместо этого нужна *нативная* Wayland-вставка (без XWayland) —
приложение само называет для этого нужным `ydotool`, с собственным экраном
диагностики (Settings → Wayland Paste Setup). На ALT p11 пакета `ydotool`
в штатных репозиториях нет (есть только в Sisyphus), поэтому здесь он не
жёсткая зависимость, а опциональная — установить и включить вручную, если
он у вас доступен:
...
`libpipewire` — это жёсткая зависимость, доступна на ALT p11 напрямую.
...
`libpipewire` объявлен жёсткой зависимостью, `ydotool` — опциональной на
ALT (жёсткой везде, где он реально доступен из коробки); см. раздел
«Вставка текста и Wayland» выше про причину этой разницы.
