...
readonly deb_image="nivora-transition-deb:$$"
readonly rpm_image="nivora-transition-rpm:$$"
readonly deb_cache_volume="nivora-transition-deb-cache-$$"
...
# old ID | new ID | command | persistent state marker | desktop/unit | icon
readonly -a transitions=(
    'clash-verge|clash-verge-rev|/usr/bin/clash-verge|/home/nivora-test/.local/share/io.github.clash-verge-rev.clash-verge-rev/nivora-transition-state|/usr/share/applications/Clash Verge.desktop|/usr/share/icons/hicolor/128x128/apps/clash-verge.png'
    'claude-code-desktop|claude-desktop|/usr/bin/claude-desktop|/home/nivora-test/.config/Claude/nivora-transition-state|/usr/share/applications/com.anthropic.Claude.desktop|/usr/share/icons/hicolor/128x128/apps/claude-desktop.png'
    'codex-app|codex|/usr/bin/codex-app|/home/nivora-test/.codex/nivora-transition-state|/usr/share/applications/codex-app.desktop|/usr/share/icons/hicolor/512x512/apps/codex-app.png'
    'luma-stplr|nivora-stplr|/usr/bin/sl|/home/nivora-test/.config/nivora-stplr/nivora-transition-state|-|-'
    'opencode|opencode|/usr/bin/opencode-desktop|/home/nivora-test/.config/opencode/nivora-transition-state|/usr/share/applications/opencode-desktop.desktop|/usr/share/icons/hicolor/128x128/apps/ai.opencode.desktop.png'
    'tailscale|tailscale|/usr/bin/tailscale|/var/lib/tailscale/nivora-transition-state|/usr/lib/systemd/system/tailscaled.service|-'
    'netbird|netbird|/usr/bin/netbird|/var/lib/netbird/nivora-transition-state|/usr/lib/systemd/system/netbird.service|-'
    'chatbox|chatbox|/usr/bin/chatbox|/home/nivora-test/.config/Chatbox/nivora-transition-state|/usr/share/applications/xyz.chatboxapp.app.desktop|/usr/share/icons/hicolor/128x128/apps/xyz.chatboxapp.app.png'
)

for command in curl docker dpkg-deb find rpm rpmbuild sha256sum sqlite3 stplr-spec tar; do
    command -v "$command" >/dev/null 2>&1 || {
        echo "Для transition-теста требуется команда ${command}" >&2
        exit 2
    }
done

[[ "$(uname -m)" == 'x86_64' ]] || {
    echo 'Transition-тест сейчас поддерживает только x86_64 runner' >&2
    exit 2
}
...
build_legacy_deb() {
    local old_name="$1"
    local root="${work_dir}/legacy-deb/${old_name}"

    install -d \
        "${root}/DEBIAN" \
        "${root}/usr/share/nivora-transition-legacy"
    printf '%s\n' \
        "Package: ${old_name}+stplr-luma" \
        'Version: 0:0.0.1-1' \
        'Architecture: all' \
        'Maintainer: Nivora tests <noreply@example.invalid>' \
        "Provides: ${old_name}" \
        'Description: Minimal legacy package fixture for Nivora transition tests' \
        >"${root}/DEBIAN/control"
    printf 'legacy fixture\n' \
        >"${root}/usr/share/nivora-transition-legacy/${old_name}"
    dpkg-deb --root-owner-group --build "$root" \
        "${work_dir}/fixtures/deb/${old_name}.deb" >/dev/null
}

build_legacy_rpm() {
    local old_name="$1"
    local topdir="${work_dir}/rpmbuild/${old_name}"
    local spec="${topdir}/SPECS/legacy.spec"
    local built
    ...
    install -d \
        ...
        "${topdir}/TMP"
    printf '%s\n' \
        "Name: ${old_name}+stplr-luma" \
        'Version: 0.0.1' \
        'Release: 1' \
        'Summary: Minimal legacy package fixture for Nivora transition tests' \
        'Group: System/Configuration/Packaging' \
        'License: MIT' \
        'BuildArch: noarch' \
        "Provides: ${old_name}" \
        '%description' \
        'Minimal legacy package fixture for Nivora transition tests.' \
        '%install' \
        'mkdir -p %{buildroot}/usr/share/nivora-transition-legacy' \
        "printf 'legacy fixture\\n' >%{buildroot}/usr/share/nivora-transition-legacy/${old_name}" \
        '%files' \
        "/usr/share/nivora-transition-legacy/${old_name}" \
        >"$spec"
    if ! rpmbuild \
    ...
    [[ -n "$built" ]]
    cp "$built" "${work_dir}/fixtures/rpm/${old_name}.rpm"
}
...
build_previous_deb() {
    ...
    install -d \
        "${root}/DEBIAN" \
        "${root}/usr/share/nivora-transition-previous"
    printf '%s\n' \
        "Package: ${system_name}" \
        'Version: 0:0.0.2-1' \
        'Architecture: all' \
        'Maintainer: Nivora tests <noreply@example.invalid>' \
        "Provides: ${old_name}, ${package}" \
        "Replaces: ${old_name}" \
        "Conflicts: ${old_name}, ${package}" \
        'Description: Previous Nivora version fixture for upgrade tests' \
        >"${root}/DEBIAN/control"
    printf 'previous fixture\n' \
        >"${root}/usr/share/nivora-transition-previous/${package}"
    dpkg-deb --root-owner-group --build "$root" \
...
build_previous_rpm() {
    ...
    printf '%s\n' \
        ...
        'Version: 0.0.2' \
        'Release: 1' \
        'Summary: Previous Nivora version fixture for upgrade tests' \
        'Group: System/Configuration/Packaging' \
        'License: MIT' \
        'BuildArch: noarch' \
        "Provides: ${old_name}" \
        ...
        "Conflicts: ${package}" \
        '%description' \
        'Previous Nivora version fixture for upgrade tests.' \
        '%install' \
        'mkdir -p %{buildroot}/usr/share/nivora-transition-previous' \
        "printf 'previous fixture\\n' >%{buildroot}/usr/share/nivora-transition-previous/${package}" \
        '%files' \
        "/usr/share/nivora-transition-previous/${package}" \
        >"$spec"
    if ! rpmbuild \
        --define "_topdir ${topdir}" \
        --define "_tmppath ${topdir}/TMP" \
...
printf '%s\n' "${transitions[@]}" >"${work_dir}/transitions.txt"

while IFS='|' read -r old_name package _; do
    build_legacy_deb "$old_name"
    build_legacy_rpm "$old_name"
    ...
    build_previous_deb \
        "$old_name" "$package" "$(dpkg-deb -f "${debs[0]}" Package)"
    build_previous_rpm \
        "$old_name" "$package" "$(rpm -qp --queryformat '%{NAME}' "${rpms[0]}")"

    for field in Provides Replaces Conflicts; do
        value="$(dpkg-deb -f "${debs[0]}" "$field")"
        metadata_contains "$value" "$old_name" || {
            echo "${package}: DEB ${field} не содержит ${old_name}" >&2
            exit 1
        }
    done
    rpm -qp --provides "${rpms[0]}" | grep -Fxq "$old_name"
    rpm -qp --obsoletes "${rpms[0]}" | grep -Fxq "$old_name"
    rpm -qp --conflicts "${rpms[0]}" | grep -Fxq "$old_name"
done <"${work_dir}/transitions.txt"
...
while IFS='|' read -r old_name package command_path state_path expected_a expected_b; do
    mapfile -t artifacts < <(find "/repo/${package}" -maxdepth 1 -type f -name '*.deb' -print)
    [[ "${#artifacts[@]}" -eq 1 ]]
    artifact="${artifacts[0]}"
    old_system_name="${old_name}+stplr-luma"
    new_system_name="$(dpkg-deb -f "$artifact" Package)"

    echo "==> DEB transition ${old_name} -> ${package}"
    dpkg -i "/fixtures/${old_name}.deb"
    ...
    ! is_installed "$old_system_name"
    is_installed "$new_system_name"
    test -x "$command_path"
    [[ "$expected_a" == '-' ]] || test -e "$expected_a"
    [[ "$expected_b" == '-' ]] || test -e "$expected_b"

    apt-get -qq remove -y "$new_system_name"
    test -f "$state_path"
    apt-get -qq install -y "/previous/${package}.deb"
    is_installed "$new_system_name"
    apt-get -qq install -y "$artifact"
    is_installed "$new_system_name"
    apt-get -qq remove -y "$new_system_name"
...
while IFS='|' read -r old_name package command_path state_path expected_a expected_b; do
    ...
    artifact="${artifacts[0]}"
    old_system_name="${old_name}+stplr-luma"
    new_system_name="$(rpm -qp --queryformat '%{NAME}' "$artifact")"

    echo "==> RPM transition ${old_name} -> ${package}"
    rpm -ivh --quiet "/fixtures/${old_name}.rpm"
    install -d "${state_path%/*}"
    printf 'keep\n' >"$state_path"
    if [[ "$state_path" == /home/nivora-test/* ]]; then
        chown -R nivora-test:nivora-test /home/nivora-test
    ...
    apt-get -qq install -y "$artifact"
    ! rpm -q "$old_system_name"
    rpm -q "$new_system_name"
    test -x "$command_path"
    [[ "$expected_a" == '-' ]] || test -e "$expected_a"
    [[ "$expected_b" == '-' ]] || test -e "$expected_b"
    ...
    apt-get -qq remove -y "$new_system_name"
    test -f "$state_path"
    apt-get -qq install -y "/previous/${package}.rpm"
    rpm -q "$new_system_name"
    apt-get -qq install -y "$artifact"
    rpm -q "$new_system_name"
    apt-get -qq remove -y "$new_system_name"
...
docker run --rm --privileged \
    -v "${repo_root}:/repo:ro" \
    -v "${work_dir}/fixtures/deb:/fixtures:ro" \
    -v "${work_dir}/fixtures/previous-deb:/previous:ro" \
    -v "${work_dir}/transitions.txt:/transitions.txt:ro" \
    -v "${work_dir}/run-deb.sh:/run-transition.sh:ro" \
    "$deb_image" \
    /run-transition.sh

docker run --rm --privileged \
    -v "${repo_root}:/repo:ro" \
    -v "${work_dir}/fixtures/rpm:/fixtures:ro" \
    -v "${work_dir}/fixtures/previous-rpm:/previous:ro" \
    -v "${work_dir}/transitions.txt:/transitions.txt:ro" \
    -v "${work_dir}/run-rpm.sh:/run-transition.sh:ro" \
    "$rpm_image" \
    /run-transition.sh

echo "OK: DEB/RPM transitions проверены для ${#transitions[@]} пакетов"
