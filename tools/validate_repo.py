...
MIGRATED_NAMES = {
    ...
    "codex": "codex-app",
    "nivora-stplr": "luma-stplr",
}

LEGACY_TEXT_ALLOWLIST = {
    Path("README.md"),
    Path("docs/migration-from-luma.md"),
    Path("docs/packages/nivora-stplr.md"),
    Path("nivora-stplr/Staplerfile"),
    Path("nivora-stplr/nivora-stplr"),
    Path("nivora-stplr/nivora-migrate-from-luma"),
    Path("tools/validate_repo.py"),
    Path("tests/test_validate_repo.py"),
    Path("tests/test_helper.sh"),
    Path("tests/test_migration.sh"),
    Path("tools/test_package_transitions.sh"),
}
...
def validate_package(package: str, errors: list[str]) -> dict[str, object]:
    ...
    expected_old = MIGRATED_NAMES.get(package)
    if expected_old:
        for field, values in (
            ("provides", provides),
            ("replaces", replaces),
            ...
            if values != [expected_old]:
                errors.append(
                    f"{package}: {field} must contain only legacy package {expected_old!r}"
                )
    else:
        if provides != [] or conflicts != []:
            errors.append(f"{package}: provides/conflicts must not contain binary aliases")
        if replaces != [package]:
            errors.append(f"{package}: replaces must contain only its prior package name")
...
def validate_repository_text(errors: list[str]) -> None:
    for path in ROOT.rglob("*"):
        ...
        if relative not in LEGACY_TEXT_ALLOWLIST and re.search(r"\bluma\b", text, re.I):
            errors.append(f"{relative}: legacy project name is outside allowlist")
